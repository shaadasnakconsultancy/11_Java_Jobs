...
public class ResultRoyaltyApplication  {
	...
	static Logger logger=LoggerFactory.getLogger(ResultRoyaltyApplication.class);
	@Autowired
	...
	@Autowired
	ResultRoyalty_repository ResultRoyalty_repository;  
	
	@Autowired
	GlobalExceptionHandler_AND_EmailSender GlobalExceptionHandler_AND_EmailSender;
	@Autowired
	ResultRoyaltyEndBoundedExcelReader ResultRoyaltyEndBoundedExcelReader;
	...
	public void run( )   {
        ...
        try {
            ...
            //2. getting all excel data in List<Result_Units> from each file one by one
             ...
             list=ResultRoyaltyEndBoundedExcelReader.readDataBetweenEndMarkers(file);
            if(list==null) {
              	 continue;
               }
            file2=file;
...
*/
Excel_Job_Tracker_TableInDB_Updater.  setFileName_FilePath_SheetName_BatchID(file,"Result Royalty",currentBatchId);
   completed=   ResultRoyalty_repository.match_FY$Actual_Remove_Insert(list,file);
              /*
               ...
               * */
            if(completed!=null) {
          	   Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Royalty",currentBatchId,"SUCCESS");
             	 
              }else if(completed==null){
              	 Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Royalty",currentBatchId,"FAIL");
              	continue;	
              }      
      ...
      //  if(excelFileList!=null) excelFileList.clear();
        System.out.println("ResultRoyalty Job Finished");
