...
public class ResultProAssApplication  {
	...
	static Logger logger=LoggerFactory.getLogger(ResultProAssApplication.class);
	@Autowired
	...
	@Autowired
	ResultProAss_repository ResultProAss_repository;  
	@Autowired
	GlobalExceptionHandler_AND_EmailSender GlobalExceptionHandler_AND_EmailSender;

	@Autowired
	ResultProAssEndBoundedExcelReader ResultProAssEndBoundedExcelReader;
	...
	public void run( )   {
        ...
        try {
            ...
            //2. getting all excel data in List<Result_Units> from each file one by one
             ...
             list=ResultProAssEndBoundedExcelReader.readDataBetweenEndMarkers(file);
            if(list==null) {
              	 continue;
               }
            file2=file;
    ...
    */
    Excel_Job_Tracker_TableInDB_Updater.  setFileName_FilePath_SheetName_BatchID(file,"Result Pro Ass", currentBatchId);
        completed= ResultProAss_repository.match_FY$Actual_Remove_Insert(list,file);
             /*
              ...
              * */
           if(completed!=null) {
         	   Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Pro Ass",currentBatchId,"SUCCESS");
            	 
             }else if(completed==null){
             	 Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Pro Ass",currentBatchId,"FAIL");
             	continue;	
             }    
...
//        	 logger.info("run() in :: > "+ ResultTransPackingApplication.class.getName());
      ...
      //  if(excelFileList!=null) excelFileList.clear();
        System.out.println("ResultProAss Job Finished");
