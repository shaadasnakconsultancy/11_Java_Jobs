...
import com.snak.MainService.FundFlowApplication;
...
import com.snak.MainService.ResultProAssApplication;
import com.snak.MainService.ResultRoyaltyApplication;
import com.snak.MainService.ResultTransPackingApplication;
import com.snak.MainService.ResultUnitsApplication;
...
public class MainBootClass implements CommandLineRunner{
	...
	@Autowired
	ResultUnitsApplication ResultUnitsApplication;
	@Autowired
	ResultPartsApplication ResultPartsApplication;
	@Autowired
	ResultPriceApplication ResultPriceApplication;
	@Autowired
	PlBudgetExportApplication PlBudgetExportApplication;
	@Autowired
	ResultProAssApplication ResultProAssApplication;
	@Autowired
	FundFlowApplication FundFlowApplication;
	
	@Autowired
	ResultTransPackingApplication ResultTransPackingApplication;
...
//this below method is used so that if internet is available only then app starts
	public static boolean isInternetAvailable() {
	    try {
	        ...
	        connection.setConnectTimeout(20000);//wait 20 sec  Time to establish connection
	        connection.connect();
	...
	public void run(String... args) throws Exception {
        ...
        try {
      	  ...
      	  batchId=    applicationPropertiesService.incrementAndStoreBatchId("job.batch.id");
        	        	  
        	  sendEmailTo=applicationPropertiesService.getProperty("send.email.to");
        	
        	//setting  these 2 so if from previous run , log are not removed from .properties, then it should get removed forst
        	...
        	ResultUnitsApplication.run();
        	  ...
        	  logAvailable=applicationPropertiesService.getProperty("logs.present");
        	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
        	...
        	//22222222222.running "ResultPartsApplication JOB"
        	ResultPartsApplication.run();
        			//check if .properties have logs.
        	  logAvailable=applicationPropertiesService.getProperty("logs.present");
        	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
        			//if "yes.logs.are.present" their means yes, then send failiur email with those logs
        	...
        	//44444444444444.running "ResultTransPackingApplication JOB"
        	ResultTransPackingApplication.run();
        			//check if .properties have logs.
        	   logAvailable=applicationPropertiesService.getProperty("logs.present");
        	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
        			//if "yes.logs.are.present" their means yes, then send failiur email with those logs
        	...
        	//5555555555.running "ResultRoyaltyApplication JOB"
        	ResultRoyaltyApplication.run();
        			//check if .properties have logs.
        	  logAvailable=applicationPropertiesService.getProperty("logs.present");
        	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
        			//if "yes.logs.are.present" their means yes, then send failiur email with those logs
        	...
        	//66666666666666.running "ResultDepApplication JOB"
        	ResultDepApplication.run();
        			//check if .properties have logs.
        	  logAvailable=applicationPropertiesService.getProperty("logs.present");
        	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
        			//if "yes.logs.are.present" their means yes, then send failiur email with those logs
...
*/    
        	...
        	ResultExpenseApplication_new.run();
        			//check if .properties have logs.
        	   logAvailable=applicationPropertiesService.getProperty("logs.present");
        	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
        			//if "yes.logs.are.present" their means yes, then send failiur email with those logs
        	...
        	applicationPropertiesService.setProperty("logs.message","");	
        	  ...
        	  System.gc();
            	...
            	//999999999999999999999999999999.running "        	  PlBudgetDomesticApplication JOB"
        	  PlBudgetDomesticApplication.run();
          			//check if .properties have logs.
          	   logAvailable=applicationPropertiesService.getProperty("logs.present");
          	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
          			//if "yes.logs.are.present" their means yes, then send failiur email with those logs
          	...
          	//10.running "        	  PlBudgetExportApplication JOB"
      	  PlBudgetExportApplication.run();
        			//check if .properties have logs.
        	   logAvailable=applicationPropertiesService.getProperty("logs.present");
        	  if("yes.logs.are.present".equalsIgnoreCase(logAvailable)) {
        			//if "yes.logs.are.present" their means yes, then send failiur email with those logs
...
//                $3 FundFlowApplication.class is having file system of fund flow
        	...
        	applicationPropertiesService.incrementAndStoreBatchId("job.batch.id");
        	applicationPropertiesService.setProperty("logs.message","");	
        	applicationPropertiesService.setProperty("logs.present","no.logs.areNot.present");
        	// logger.info("run() in :: > "+ ResultUnitsApplication.class.getName());
        	e.printStackTrace();
System.out.println("exception in MainBootClass run() method");
        	 
        	  //new PrintLog_AND_SendEmail_Exception("MainBootClass.class", "run()", "", "exception in MainBootClass run() method", "");
emailSender.sendEmail(sendEmailTo, "problem while running java jobs", "please check logs and other emails");
        }
