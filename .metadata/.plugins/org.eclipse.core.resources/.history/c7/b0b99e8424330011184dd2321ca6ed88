...
System.out.println("email sent");
	...
	*/
    ...
    try {
        ...
        System.out.println("Exception in emailSender.java when sending email");
        e.printStackTrace();
    }
}


public String sendSuccessEmailWithExcelJobTracketData(String jobName,String batchId,String sendEmailTo){
	...
	if(!pstmt.getResultSet().next()) {
	 ...
	 /*
	      GlobalExceptionHandler_AND_EmailSender.handleException(  new PrintLog_AND_SendEmail_Exception(
                    "ResultExpense_repository_new.class",
...
//	}
	...
	sql = "SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Excel_Job_Tracker'";
	pstmt = conn.prepareStatement(sql);
	...
	if (!rs.next()) {
	    ...
	    logger.info("Table does not exist in FinanceDashboard DB with name -> Excel_Job_Tracker  at the point of taking data and attaching to email in emailSender.class at sendSuccessEmailWithExcelJobTracketData() method" );
	      throw new Exception();
	    /*
	    GlobalExceptionHandler_AND_EmailSender.handleException(
	        new PrintLog_AND_SendEmail_Exception(
	            ...
	            "match_FY$Actual_Remove_Insert()",
	            file.getName(),
	            "Table does not exist in SQL Server with name -> Excel_Job_Tracker",
	            ""
	        )
	    );
 ...
 */
	    ...
	    System.out.println("Table Excel_Job_Tracker is present");
	}
	//END//checking below is that tabel with "Excel_Job_Tracker" name exist or not
	...
	InsertedRows	int
	DeletedRows	int
	ErrorMessages	nvarchar
	BatchID	bigint
	*/
//3.	 storing excel data of fy and Actual in table in batches of 100
		    sql="select BatchID,FileName,	FullPath,ProcessedAt,SheetName,InsertedRows,DeletedRows,ErrorMessages from Excel_Job_Tracker"
		    		+ " where SheetName=? AND BatchID=?  ";
...
//		Set parameters BEFORE executing
		...
		if(b) {
			...
			header.setFullPath("FullPath");
			header.setProcessedAt("ProcessedAt");
			header.setSheetName("SheetName");
			header.setInsertedRows("InsertedRows");
			header.setDeletedRows("DeletedRows");
			header.setErrorMessages("ErrorMessages");
			...
			if(resultset.next()) {
				do {
				    ...
				    obj.setBatchID(String.valueOf(resultset.getInt(1)));
				    obj.setFileName(resultset.getString(2));
				    obj.setFullPath(resultset.getString(3));
				    obj.setProcessedAt(String.valueOf(resultset.getTimestamp(4)));
				    obj.setSheetName(resultset.getString(5));
...
//adding these excel_job_tracker data to email and sending it
		String emailBody = convertToHTMLTable(list);
		sendEmail(sendEmailTo, jobName +" job success for batchID "+batchId, emailBody);
		list.clear();
...
/* COMMIT TRANSACTION */
		...
		//sending success email without excel_job_tracker table data
		...
		e.printStackTrace() ;
		   logger.info("Exception in doing DB task in sendSuccessEmailWithExcelJobTracketData() method > "+ emailSender.class.getName()+ "  ");
...
public String sendFailEmailWithExcelJobTracketDataANDLogMessages(String jobName,String batchId,String sendEmailTo,String LogMessage){
	int totalDeletedRows=0;
...
//adding these excel_job_tracker data to email and sending it
		String emailBody = convertToHTMLTable(list);
		sendEmail(sendEmailTo, jobName +" job fail status for batchID "+batchId, emailBody+"\n\n\n log messages where problem occur in programm are ->\n\n \n"+LogMessage);
		list.clear();
...
public String convertToHTMLTable(List<excelJobTrackerDTO> list) {
    ...
    for (int i = 0; i < list.size(); i++) {
        ...
        excelJobTrackerDTO obj = list.get(i);

        sb.append("<tr>");

        sb.append("<td>").append(obj.getBatchID()).append("</td>");
        sb.append("<td>").append(obj.getFileName()).append("</td>");
        sb.append("<td>").append(obj.getFullPath()).append("</td>");
        sb.append("<td>").append(obj.getProcessedAt()).append("</td>");
        sb.append("<td>").append(obj.getSheetName()).append("</td>");
        sb.append("<td>").append(obj.getInsertedRows()).append("</td>");
        sb.append("<td>").append(obj.getDeletedRows()).append("</td>");
        sb.append("<td>").append(obj.getErrorMessages()).append("</td>");
