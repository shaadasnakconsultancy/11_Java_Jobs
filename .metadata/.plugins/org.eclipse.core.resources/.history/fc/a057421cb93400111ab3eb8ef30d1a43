...
public class PlBudgetDomesticApplication  {
	...
	static Logger logger=LoggerFactory.getLogger(PlBudgetDomesticApplication.class);
	@Autowired
	...
	@Autowired
	PlBudgetDomestic_repository PlBudgetDomestic_repository;  
	@Autowired
	GlobalExceptionHandler_AND_EmailSender GlobalExceptionHandler_AND_EmailSender;

	@Autowired
	PlBudgetDomesticEndBoundedExcelReader PlBudgetDomesticEndBoundedExcelReader;
	...
	public void run( )   {
        ...
        try {
            ...
            //2. getting all excel data in List<Result_Units> from each file one by one
             ...
             list=PlBudgetDomesticEndBoundedExcelReader.readDataBetweenEndMarkers(file);
            if(list==null) {
              	 continue;
               }
            file2=file;
//            list.stream().forEach(n->System.out.println(n));
    ...
    */
    Excel_Job_Tracker_TableInDB_Updater.  setFileName_FilePath_SheetName_BatchID(file,"Pl Budget Domestic", currentBatchId);
        completed= PlBudgetDomestic_repository.match_FY$Actual_Remove_Insert(list,file);
             /*
              ...
              * */
           if(completed!=null) {
         	   Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Pl Budget Domestic",currentBatchId,"SUCCESS");
            	 
             }else if(completed==null){
             	 Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Pl Budget Domestic",currentBatchId,"FAIL");
             	continue;	
             }    
...
//        	 logger.info("run() in :: > "+ ResultTransPackingApplication.class.getName());
      ...
      //  if(excelFileList!=null) excelFileList.clear();
        System.out.println("PlBudgetDomesticApplication Job Finished");
