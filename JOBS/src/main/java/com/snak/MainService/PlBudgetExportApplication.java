...
import com.snak.Repository.PlBudgetExport_repository;
import com.snak.Services.Dynamic_File_System;
import com.snak.Services.LoggerCLass;
import com.snak.Services.PlBudgetExportEndBoundedExcelReader;
...
@Service
public class PlBudgetExportApplication  {

	static Logger logger=LoggerFactory.getLogger(PlBudgetExportApplication.class);
	@Autowired
	...
	@Autowired
	PlBudgetExport_repository PlBudgetExport_repository;  
	@Autowired
	GlobalExceptionHandler_AND_EmailSender GlobalExceptionHandler_AND_EmailSender;

	@Autowired
	PlBudgetExportEndBoundedExcelReader PlBudgetExportEndBoundedExcelReader;
	...
	public void run( )   {
    	...
    	File file2=null;
    	List<PlBudgetExport_DTO> list=null;
      ...
      */
      Excel_Job_Tracker_TableInDB_Updater.  setFileName_FilePath_SheetName_BatchID(file,"Pl Budget Export", currentBatchId);
             list=PlBudgetExportEndBoundedExcelReader.readDataBetweenEndMarkers(file);
            if(list==null) {
            	Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Pl Budget Export",currentBatchId,"FAIL");
              	 continue;
               }
            file2=file;
        ...
        completed= PlBudgetExport_repository.match_FY$Actual_Remove_Insert(list,file);
             /*
              ...
              * */
           if(completed!=null) {
         	   Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Pl Budget Export",currentBatchId,"SUCCESS");
...
//        	 logger.info("run() in :: > "+ ResultTransPackingApplication.class.getName());
        	...
        	GlobalExceptionHandler_AND_EmailSender.handleException(  new PrintLog_AND_SendEmail_Exception("PlBudgetExportApplication.class", "run() ", file2!=null? file2.getName():"" , " exception while reading data from this excel or at doing DB operation of this file ",e.toString()+ " "+e.getMessage()));
        	e.printStackTrace();
        }
      //  if(excelFileList!=null) excelFileList.clear();
        System.out.println("PlBudgetExportApplication Job Finished");
