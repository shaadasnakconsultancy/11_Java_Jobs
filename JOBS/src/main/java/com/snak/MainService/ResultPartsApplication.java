...
import com.snak.Repository.ResultParts_repository;
...
@Service
public class ResultPartsApplication  {

	static Logger logger=LoggerFactory.getLogger(ResultPartsApplication.class);
	@Autowired
	LoggerCLass LoggerCLass;
	@Autowired
	GlobalExceptionHandler_AND_EmailSender GlobalExceptionHandler_AND_EmailSender;
	@Autowired
	ResultParts_repository ResultParts_repository;  
	@Autowired
	...
	@Autowired
	ResultPartsEndBoundedExcelReader ResultPartsEndBoundedExcelReader;
	...
	public void run( )   {
        ...
        System.out.println("ResultPartsApplication Job Started");
        //emailSender.sendEmail("mdshaad024@gmail.com", "subject", "message");
        //System.exit(0);
        File file2=null;
        List<ResultParts> list=null;
      ...
      */
       Excel_Job_Tracker_TableInDB_Updater.  setFileName_FilePath_SheetName_BatchID(file,"Result Parts", currentBatchId);
            	  
            	  
            list=ResultPartsEndBoundedExcelReader.readDataBetweenEndMarkers(file);
            if(list==null) {
            	Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Parts",currentBatchId,"FAIL");
              	 continue;
               }            
            ...
            completed=   ResultParts_repository.match_FY$Actual_Remove_Insert(list,file);
             ...
             * */
          if(completed!=null) {
        	   Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Parts",currentBatchId,"SUCCESS");
      ...
      //  File_System.moveFileToArchive(file, file);
        //if we get true means file is being moved successfully , mean all previous steps have been completed and now we can send success email
          ...
          if(list!=null)  list.clear(); 
        	...
        	GlobalExceptionHandler_AND_EmailSender.handleException(  new PrintLog_AND_SendEmail_Exception("ResultPartsApplication.class", "run() ", file2!=null? file2.getName():"" ," exception while reading data from this excel or at doing DB operation of this file",e.toString()+ " " +e.getMessage()));
        	e.printStackTrace();
        }

     //   if(excelFileList!=null) excelFileList.clear();
        System.out.println("ResultParts Job Finished");
