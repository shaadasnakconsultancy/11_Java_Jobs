...
import com.snak.Repository.ResultPrice_repository;
import com.snak.Services.Dynamic_File_System;
import com.snak.Services.LoggerCLass;
import com.snak.Services.ResultPriceEndBoundedExcelReader;
...
@Service
public class ResultPriceApplication  {

	static Logger logger=LoggerFactory.getLogger(ResultPriceApplication.class);
	@Autowired
	...
	@Autowired
	ResultPrice_repository ResultPrice_repository;  
	
	@Autowired
	GlobalExceptionHandler_AND_EmailSender GlobalExceptionHandler_AND_EmailSender;
	@Autowired
	ResultPriceEndBoundedExcelReader ResultPriceEndBoundedExcelReader;
     ...
     completed=  ResultPrice_repository.match_FY$Actual_Remove_Insert(list,file);
         ...
         // File_System.moveFileToArchive(file, file);
          ...
          if(list!=null)list.clear(); 
        	...
        	// logger.info("run() in :: > "+ ResultPriceApplication.class.getName());
        	GlobalExceptionHandler_AND_EmailSender.handleException(  new PrintLog_AND_SendEmail_Exception("ResultPriceApplication.class", "run() ", file2!=null? file2.getName():"" , " exception while reading data from this excel or at doing DB operation of this file",e.toString()+ " " +e.getMessage()));
        	e.printStackTrace();
        	 // LoggerCLass.printStackTraceToLogs(ResultUnitsApplication.class.getName(),"run", e);
        }

     //   if(excelFileList!=null) excelFileList.clear();
        System.out.println("ResultPrice Job Finished");
