...
import com.snak.Repository.ResultTransPacking_repository;
import com.snak.Services.Dynamic_File_System;
import com.snak.Services.LoggerCLass;
import com.snak.Services.ResultTransPackingEndBoundedExcelReader;
...
@Service
public class ResultTransPackingApplication  {

	static Logger logger=LoggerFactory.getLogger(ResultTransPackingApplication.class);
	@Autowired
	...
	@Autowired
	ResultTransPacking_repository ResultTransPacking_repository;  
	@Autowired
	GlobalExceptionHandler_AND_EmailSender GlobalExceptionHandler_AND_EmailSender;

	@Autowired
	ResultTransPackingEndBoundedExcelReader ResultTransPackingEndBoundedExcelReader;
	...
	public void run( )   {
    	...
    	File file2=null;
    	List<Result_Trns_Packing> list=null;
      ...
      */
      Excel_Job_Tracker_TableInDB_Updater.  setFileName_FilePath_SheetName_BatchID(file,"Result Trans Packing", currentBatchId);
             list=ResultTransPackingEndBoundedExcelReader.readDataBetweenEndMarkers(file);
            if(list==null) {
            	Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Trans Packing",currentBatchId,"FAIL");
              	 continue;
               }
            file2=file;
        ...
        completed= ResultTransPacking_repository.match_FY$Actual_Remove_Insert(list,file);
             /*
              ...
              * */
           if(completed!=null) {
         	   Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Trans Packing",currentBatchId,"SUCCESS");
...
//        	 logger.info("run() in :: > "+ ResultTransPackingApplication.class.getName());
        	...
        	GlobalExceptionHandler_AND_EmailSender.handleException(  new PrintLog_AND_SendEmail_Exception("ResultTransPackingApplication.class", "run() ", file2!=null? file2.getName():"" , " exception while reading data from this excel or at doing DB operation of this file ",e.toString()+ " "+e.getMessage()));
        	e.printStackTrace();
        }
      //  if(excelFileList!=null) excelFileList.clear();
        System.out.println("ResultTransPacking Job Finished");
