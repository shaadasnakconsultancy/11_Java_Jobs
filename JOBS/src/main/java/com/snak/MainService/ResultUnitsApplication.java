...
import com.snak.Repository.Resultunits_repository;
...
@Service
public class ResultUnitsApplication  {

	static Logger logger=LoggerFactory.getLogger(ResultUnitsApplication.class);
	@Autowired
	...
	@Autowired
	Resultunits_repository Resultunits_repository;  
	@Autowired
	...
	public void run( )   {
        ...
        try {
        	List<Result_Units_DTO> list=null;
    ...
    completed=  Resultunits_repository.match_FY$Actual_Remove_Insert(list,file);
               ...
               * */
            if(completed!=null) {
          	   Excel_Job_Tracker_TableInDB_Updater.  updatingExcelJobTrackr_ErrorMessagesColumns(file,"Result Units",currentBatchId,"SUCCESS");
        ...
        //  File_System.moveFileToArchive(file, file);
          //clearing the list so only next excel data will be available in lsit not data from all excel files
        	...
        	GlobalExceptionHandler_AND_EmailSender.handleException(  new PrintLog_AND_SendEmail_Exception("ResultUnitsApplication.class", "run() ",file2!=null? file2.getName():"" , " exception while reading data from this excel or at doing DB operation of this file",e.toString()+ " "+e.getMessage()));
        	e.printStackTrace();
        }
      //  if(excelFileList!=null) excelFileList.clear();
        System.out.println("ResultUnits Job Finished");
