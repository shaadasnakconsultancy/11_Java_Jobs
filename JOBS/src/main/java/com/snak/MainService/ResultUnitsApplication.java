    ...
    completed=  Resultunits_repository.match_FY$Actual_Remove_Insert(list,file);
        ...
        //  File_System.moveFileToArchive(file, file);
          //clearing the list so only next excel data will be available in lsit not data from all excel files
        	...
        	GlobalExceptionHandler_AND_EmailSender.handleException(  new PrintLog_AND_SendEmail_Exception("ResultUnitsApplication.class", "run()", file2.getName(), "exception while reading data from this excel or at doing DB operation of this file",e.toString()+ " "+e.getMessage()));
        	e.printStackTrace();
        }
